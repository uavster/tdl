...
extern void GenericTriangleMapper(SLI *sbuffer, RENDERPOLY *p)
{
  ...
  for(i=0;i<=p->N;i++)
  {
    sp.SPVars[i*2+1]= ancho*(temp * (p3->SpanVars.k[i] - p1->SpanVars.k[i]) + (p1->SpanVars.k[i] - p2->SpanVars.k[i]));
    ks1[i]=   65536.0 * p1->SpanVars.k[i];
  }
  sp.SPN=p->N+1;
  sp.SPType=p->Type;
  switch(p->Type)
  ...
  */
  if( ancho1 > 0)
  {
      ...
      if((p2y - p1y)!=0)
      {
        ...
        for(i=0;i<=p->N;i++)
        {
          deltas1[i]= temp * (p2->SpanVars.k[i] - p1->SpanVars.k[i]);
        }
      ...
      for(i=0;i<=p->N;i++)
      {
        deltas1[i]= temp * (p3->SpanVars.k[i] - p2->SpanVars.k[i]);
      }
      x1=p2->ImageProjection.x;
/*      for(i=0;i<=p->N;i++)
      {
        ks1[i]=   65536.0 * p2->SpanVars.k[i]; pasado a abajo
      }*/
      ...
      for(i=0;i<=p->N;i++)
      {
        ks1[i]=65536.0 * p2->SpanVars.k[i]+((float)y-p2->ImageProjection.y)*deltas1[i];
      }
      y2=(int)p3y;
      if(y2>sbuffer->SLIClip.SLRR2.SLPY) y2=sbuffer->SLIClip.SLRR2.SLPY;
  ...
  else
  {
      ...
      for(i=0;i<=p->N;i++)
      {
        deltas1[i]= temp * (p3->SpanVars.k[i] - p1->SpanVars.k[i]);
      }
